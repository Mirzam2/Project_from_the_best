...
BlackJack::BlackJack(std::unique_ptr<Player[]> Players, Host Croupier, int numofplayers) : Game(std::move(Players), Croupier, numofplayers)
{
    ...
    bets = std::make_unique<unique_ptr<int[]>[]>(numofplayers);
    for (int i = 0; i < numofplayers; ++i)
    {
        bets[i] = std::make_unique<int[]>(2);
    }
    ...
    hands = std::make_unique<unique_ptr<Hand[]>[]>(numofplayers);
    for (int i = 0; i < numofplayers; ++i)
    {
        hands[i] = std::make_unique<Hand[]>(2);
    }
}
